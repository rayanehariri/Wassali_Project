...
class User:
    ...
    @staticmethod
    def mark_onboarding_done(user_id: str) -> dict:
        """Mark deliverer onboarding as complete and set status to pending."""
        try:
            users_collection.update_one(
                {"_id": user_id},
                {"$set": {"onboardingDone": True, "status": Status.PENDING.value}},
            )
