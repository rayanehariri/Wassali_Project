...
import {
  ...
  XCircle, Headphones, RefreshCw, Plus, Minus, Locate, MapPin, Clock,
} from "lucide-react";
...
// ── Fix Leaflet's default marker icons (broken in Webpack/Vite) ──
...
const riderIcon = L.divIcon({
  ...
  html: `<div style="
    ...
    box-shadow: 0 0 0 6px rgba(59,130,246,0.25), 0 8px 24px rgba(59,130,246,0.45);
    font-size: 20px;
  ">🛵</div>`,
  iconSize:   [42, 42],
  iconAnchor: [21, 21],
    ...
    <div style={{ width: "100%", height: "100%", position: "relative" }}>
      ...
      >
        ...
        <Marker
          ...
          icon={restaurantIcon}
        >
          <Popup>{restaurantLocation.label}</Popup>
        </Marker>
        ...
        <Marker
          ...
          icon={riderIcon}
        >
          <Popup>Rider: {order.deliverer.name}</Popup>
        </Marker>
      </MapContainer>

      {/* Live tracking badge (overlaid on map) */}
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "6px", marginBottom: "2px" }}>
          ...
          <span style={{ fontSize: "10px", fontWeight: 700, color: "#10b981", letterSpacing: "1px" }}>
            LIVE TRACKING
          </span>
        </div>
        ...
        <p style={{ fontSize: "11px", color: "#64748b", margin: 0 }}>
          Estimated arrival at {order.estimatedArrival}
        </p>
      </div>
    </div>
  );
...
export default function OrderDetail() {
  ...
  async function handleForceUpdate() {
    ...
    try {
      ...
      setOrder((prev) => ({ ...prev, status: updated.status, timeline: updated.timeline }));
    } finally {
      setActing(false);
    }
  }
  ...
  return (
    <div style={{ display: "flex", height: "100vh", background: "#0f1117", overflow: "hidden" }}>
      ...
      <div style={{
        ...
        background: "#0f1117", borderLeft: "1px solid #1e2d3d",
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 24px", display: "flex", flexDirection: "column", gap: "16px" }}>
          ...
          {/* Involved Parties */}
          <section style={{ background: "#161f2e", borderRadius: "14px", border: "1px solid #1e2d3d", padding: "16px" }}>
            <p style={{ fontSize: "10px", fontWeight: 700, color: "#475569", letterSpacing: "1px", marginBottom: "14px" }}>
              INVOLVED PARTIES
            </p>
            {[
              ...
              <div key={role} style={{
                ...
                paddingBottom: role === "Customer" ? "14px" : 0,
                marginBottom:  role === "Customer" ? "14px" : 0,
                ...
                <div style={{ display: "flex", gap: "6px" }}>
                  {[Phone, MessageSquare].map((Icon, i) => (
                    <button key={i} style={{
                      ...
                      background: "#1e2d3d", border: "1px solid #334155",
          ...
          {/* Delivery Timeline */}
          <section style={{ background: "#161f2e", borderRadius: "14px", border: "1px solid #1e2d3d", padding: "16px" }}>
            <p style={{ fontSize: "10px", fontWeight: 700, color: "#475569", letterSpacing: "1px", marginBottom: "16px" }}>
              DELIVERY TIMELINE
            </p>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                      ...
                      {!isLast && (
                        <div style={{
                          ...
                          background: step.done ? "#10b981" : "#1e2d3d",
                          margin: "3px 0",
                        }} />
                      )}
                    </div>
                    <div style={{ paddingBottom: isLast ? 0 : "16px" }}>
                      <p style={{ fontSize: "13px", fontWeight: 600, margin: 0, color: step.done || step.active ? "white" : "#334155" }}>
                        {step.label}
                      </p>
                      {step.time && (
                        <p style={{ fontSize: "11px", color: "#64748b", margin: "2px 0 0" }}>{step.time}</p>
                      )}
                    </div>
                  </div>
                );
              ...
              {(order.items || []).map((item, i) => (
                <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
                    <span style={{
                      background: "#1e2d3d", borderRadius: "6px",
        ...
        {/* Footer Actions */}
        <div style={{ padding: "16px 24px", borderTop: "1px solid #1e2d3d", display: "flex", flexDirection: "column", gap: "10px" }}>
